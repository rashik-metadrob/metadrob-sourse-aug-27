...
const dataToCopy = require('../data-to-copy/metadrob-test.projects.json')
...
// mongoimport --uri="mongodb://admin:admin@metadrob.com:17017/Metadrob?authSource=Metadrob&readPreference=primary&appname=MongoDB%20Compass%20Community&ssl=false" --db=Metadrob --collection=projects --mode=upsert --type=json --jsonArray --file="C:\PROJECT\Metadrob shopify\metadrob\server\src\copy-from-testversion\data-to-copy\metadrob-test.projects.json"
...
mongoose.connect(config.mongoose.url, config.mongoose.options).then(async () => {
    ...
    for(let i = 0; i < dataToCopy.length; i++){
        ...
        const filePath = _.get(dataToCopy, [i, 'template'], '')

        const cmdThumnailCode = `cp ${drobverseUploadFolder}/${image} ${metadrobUploadFolder}/${image}`
