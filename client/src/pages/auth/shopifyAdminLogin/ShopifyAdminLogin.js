...
const ShopifyAdminLogin = () => {
  ...
  const onLoginSuccess = (data) => {
    ...
    if(_.get(data.user, 'role', '') === USER_ROLE.ADMIN){
        ...
        navigate(routesConstant.shopifyAdminHome.path)
    } else {
        notification.error({
          message: "You aren't the Admin of Metadrob Shopify!"
        })
    }
  }

  return (
    <Row className="shopify-admin-login__container">
      ...
      <Col lg={9} md={10} sm={24} xs={24} className="right-col">
        <div className="login-form">
          <div className="login-title">Login to Shopify Admin Dashboard</div>
          <div className="sub__login-title">
            This is your Shopify Admin dashboard login Panel. Enter Your Login
            Credentials to login to your Shopify Admin dashboard
          </div>
...
export default ShopifyAdminLogin
