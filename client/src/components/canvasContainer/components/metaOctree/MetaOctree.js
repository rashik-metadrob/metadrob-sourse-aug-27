...
class MetaOctree {
		...
		for ( let x = 0; x < 2; x ++ ) {
			...
			for ( let y = 0; y < 2; y ++ ) {
				...
				for ( let z = 0; z < 2; z ++ ) {
					...
					subTrees.push( new MetaOctree( box ) );
	...
	fromGraphNode( group, isCollider = false ) {

		group.updateWorldMatrix( true, true );

		group.traverse( ( obj ) => {

			if ( obj.isMesh === true || obj.type === "BoxHelper"  ) {
				...
				if ( this.layers.test( obj.layers ) ) {
					...
					let step = 1;
					if(positionAttribute.count > 5000 && !isCollider){
						step = Math.ceil(positionAttribute.count / 5000);
					}

					for ( let i = 0; i < positionAttribute.count; i += 3 * step ) {
