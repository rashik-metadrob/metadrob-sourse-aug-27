...
import { deleteProject, getListProject, getListProjectByAdmin, updateProjectById } from "../../api/project.api";
import { EDIT_TEMPLATE_MODE, PROJECT_TYPE, SERVER_DATE_FORMAT, TEMPLATE_TABLE_DATE_FORMAT } from "../../utils/constants";
import { getStorageUserDetail } from "../../utils/storage";
import EyeIcon from "../../assets/icons/EyeIcon";
import TrashIcon from "../../assets/icons/TrashIcon";
import CalendarIcon from "../../assets/icons/CalendarIcon";
import EditIcon from "../../assets/icons/EditIcon"
import ModalEditTemplate from "../adminComponents/modalEditTemplate/ModalEditTemplate"
...
const TemplateTable = ({
  search = ""
    ...
    const columns = [
      {
        ...
        align: 'left',
        width: "70px",
        render: (text, record) => <Checkbox checked={!_.get(record, ['isDisabled'], false)} onChange={(e) => {onLockTemplateChange(record, !e.target.checked)}}/>,
      },
      {
        title: 'Thumbnail',
        dataIndex: 'image',
        ...
        render: (text, record, index) => <>
          <div className="flex gap-[12px] justify-center">
            <div className="flex flex-col items-center gap-[7px]">
              <button className="btn-action" onClick={() => {navigate(`/${isShopifyAdminLocation(location) ? 'admin-shopify' : 'admin'}/template/${EDIT_TEMPLATE_MODE.EDIT_TEMPLATE}/${record.id}`)}}>
                <EyeIcon/>
              </button>
              <button className="btn-action" onClick={() => {navigate(`/${isShopifyAdminLocation(location) ? 'admin-shopify' : 'admin'}/template/${EDIT_TEMPLATE_MODE.CLONE_TEMPLATE}/${record.id}`)}}>
                <CloneIcon/>
              </button>
    ...
    const onLockTemplateChange = (record, value) => {
      setIsLoading(true)
      updateProjectById(record.id, { isDisabled: value}).then(rs => {
        setIsLoading(false)
        setListTemplates(data => data.map(el => {
          if(el.id === record.id) {
            el.isDisabled = value
          }
    ...
    const loadData = () => {
      ...
      timeoutRef.current = setTimeout(() => {
        ...
        getListProjectByAdmin(filterData).then((rs) => {
          ...
          notification.error({
            message: _.get(err, ['response', 'data', 'message'], `Can't get template data!`)
          })
          setIsLoading(false)
        })
      }, 300)
    ...
    const onDeleteRecord = (record) => {
      Modal.confirm({
        title: "Are you sure to delete template",
        centered: true,
